export function addTrackingToEmail(
  ...
  trackedContent = trackedContent.replace(
    ...
    (match, url) => {
      ...
      const trackingUrl = `${baseUrl}/api/track/click?c=${encodeURIComponent(
        campaignId
      )}&u=${encodeURIComponent(contactId)}&url=${encodedUrl}&t=${Date.now()}`;
...
export function createUnsubscribeUrl(
  ...
  const campaignParam = campaignId
    ? `&campaign=${encodeURIComponent(campaignId)}`
    : "";
  return `${baseUrl}/api/unsubscribe?email=${encodedEmail}${campaignParam}`;
}
